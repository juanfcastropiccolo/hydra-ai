...
- **Importar contexto (004):** el texto se pega con **bracketed paste** (`@shared/paste`) y **nunca** se escribe `\r` — enviar es siempre el Enter del usuario. Solo se pega si la sesión destino sigue `idle` (guarda evaluada en `Pane`, donde vive el `ptyId`). `claude -p` solo se invoca desde `ClaudeCli`; el encuadre del bloque va en primera persona del usuario (un wrapper imperativo fue rechazado como prompt injection en el spike). `agents --json` devuelve `state: "done"` con `pid` vivo tras cada turno: eso es idle, no finalizada.
