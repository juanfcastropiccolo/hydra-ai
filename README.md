...
- **Graph Know (feature 006):** índice de conocimiento sobre todo tu trabajo pasado (mismos transcripts que Analytics): búsqueda léxica + grafo (proyectos, sesiones, archivos, temas, hechos) con multi-salto, **fichas por sesión** generadas con `claude -p` (haiku por defecto; automáticas con toggle, el backlog inicial pide confirmación con costo) y **MCP server local `hydra-know`** (HTTP en `127.0.0.1:4855`, solo lectura) con las tools `search_past_work` y `get_session_context`. "Conectar con Claude Code" lo registra con `claude mcp add -s user` (para quitarlo a mano: `claude mcp remove hydra-know -s user`). En sesiones no interactivas hay que permitir la tool (`--allowedTools mcp__hydra-know__search_past_work`); en las interactivas Claude Code pregunta una vez. Con Hydra cerrada la tool no responde; vuelve al abrirla.
- **Config (feature 007):** ítem _Config_ del sidebar o **⌘,** → pantalla de preferencias estilo Discord: **Perfil** (nombre e iniciales del avatar), **Apariencia** (fuente de las terminales en vivo, zoom por defecto, color de acento), **Sesiones** (modelo / esfuerzo / modo de permisos que Hydra pasa a `claude --bg`, patrón del nombre sugerido, confirmar al terminar), **Contexto e IA** (modelo económico, fichas automáticas, presupuesto por ficha, puerto y conexión del MCP, rango de Analytics), **Analytics** (tabla de precios editable) y **Mantenimiento** (CLI detectado, carpeta de datos, reindexar, borrar fichas/caché, exportar/importar `hydra.json` con `.bak`, errores recientes). Toggles y selects guardan al instante; los campos de texto con Guardar/Descartar.
- QA manual y desvíos: `docs/qa-001.md`, `docs/qa-002.md`, `docs/qa-004.md`, `docs/qa-005.md`, `docs/qa-006.md`, `docs/qa-007.md`. Spikes técnicos: `docs/spike-001-attach.md`, `docs/spike-004-context.md`.
