...
- [ ] **1.** Inicializar proyecto electron-vite + React + TS strict — done when: `npm run dev` abre una ventana vacía "Hydra"; `npm run build` genera `out/`; `tsc --noEmit` pasa; estructura `src/main`, `src/preload`, `src/renderer`, `src/shared` creada.
