...
- [x] **14.** QA manual `docs/qa-004.md` (AC-1/2/5/6/8 con sesiones reales de este repo, origen larga: tiempo/costo observados, `.app` empaquetada con OAuth, modelo inválido en `hydra.json`) + empaquetar `Hydra.app` 0.3.0 — done when: checklist completo por Juan; fallos → tasks de fix.
- [x] **15.** Cierre: README (sección "Importar contexto"), CLAUDE.md (módulo `context/`, regla: nunca escribir `\r` al pegar; bracketed paste; `-p` solo vía `ClaudeCli`), `.sdd/README.md` (004 complete; 007 hereda `ui.importContext.model` y `--max-budget-usd`), `status.json` → `complete`, tag `v0.3.0` — done when: hecho y pusheado.
